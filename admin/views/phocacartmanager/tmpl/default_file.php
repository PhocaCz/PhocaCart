...
if ($this->manager == 'productimage' || $this->manager == 'categoryimage') {
	...
	*/
	...
	$jsParams = '{'
		...
		.' requesturl: \''.$url.'\','
		.' requestmsg: \''.strip_tags(addslashes(Text::_('COM_PHOCACART_CHECKING_IMAGE_THUMBNAIL_PLEASE_WAIT'))).'\''
		.'}';
...
if ($this->manager == 'filemultiple') {
	...
	if (($group['i'] == 1) && ($ext == 'png' || $ext == 'jpg' || $ext == 'gif' || $ext == 'jpeg' || $ext = 'webp') ) {
		...
		. HTMLHelper::_( 'image', str_replace( '../', '', $this->_tmp_file->path_without_name_relative), Text::_('COM_PHOCACART_INSERT'), array('title' => Text::_('COM_PHOCACART_INSERT_IMAGE'), 'width' => '16', 'height' => '16'))
		.'</a>'
		.' <td>'
		.'<a href="#" onclick="'.$onclick.'">'
		. $this->_tmp_file->name
		...
		.'<td>'
		.'<a href="#" onclick="'.$onclick.'">'
		. HTMLHelper::_( 'image', $this->t['i'].'icon-16-file-insert.png', '', Text::_('COM_PHOCACART_INSERT_FILENAME'))
		.'</a>'
		.' <td>'
		.'<a href="#" onclick="'.$onclick.'">'
		. $this->_tmp_file->name
