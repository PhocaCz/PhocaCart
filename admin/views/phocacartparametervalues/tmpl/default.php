...
echo $r->selectFilterPublished('JOPTION_SELECT_PUBLISHED', $this->state->get('filter.published'));
echo $r->selectFilterParameter(PhocacartParameter::options($this->t['o']), 'COM_PHOCACART_SELECT_PARAMETER', $this->state->get('filter.parameter_id'));
echo $r->endFilterBar();
...
echo '<th class="ph-title">'.HTMLHelper::_('searchtools.sort',  	$this->t['l'].'_PARAMETER', 'a.parameter_id', $listDirn, $listOrder ).'</th>'."\n";
...
$linkEditP		= Route::_( $urlEditP. $item->parameter_id );
...
$paramO = '';
if ($canCreate || $canEdit) {
	$paramO .= '<a href="'. Route::_($linkEditP).'">'. $this->escape($item->parameter_title).'</a>';
} else {
