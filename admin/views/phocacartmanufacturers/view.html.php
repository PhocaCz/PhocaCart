...
class PhocaCartCpViewPhocacartManufacturers extends JViewLegacy
{
	...
	function addToolbar() {
		...
		$canDo	= $class::getActions($this->t, $state->get('filter.manufacturer_id'));

		$bar = JToolbar::getInstance('toolbar');

		JToolbarHelper::title( JText::_( $this->t['l'].'_MANUFACTURERS' ), 'home' );
		...
		if ($canDo->get('core.delete')) {
			JToolbarHelper::deleteList( $this->t['l'].'_WARNING_DELETE_ITEMS', 'phocacartmanufacturers.delete', $this->t['l'].'_DELETE');
		}

		$dhtml = '<button onclick="javascript:if(document.adminForm.boxchecked.value==0){alert(\''.JText::_('COM_PHOCACART_WARNING_COUNT_PRODUCTS_MAKE_SELECTION').'\');}else{Joomla.submitbutton(\'phocacartmanufacturer.countproducts\');}" class="btn btn-small button-plus"><i class="icon-plus" title="'.JText::_($this->t['l'].'_COUNT_PRODUCTS').'"></i> '.JText::_($this->t['l'].'_COUNT_PRODUCTS').'</button>';
