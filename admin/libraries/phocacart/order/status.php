...
class PhocacartOrderStatus
{
    ...
    private static function sendOrderEmail(object $order, PhocacartOrderView $orderView, array $status, array $addresses, string $orderToken, bool $notifyUser, bool $notifyOthers, $emailSend, $emailSendFormat): int
    {
        ...
        if (!MailHelper::isEmailAddress($recipient) && !MailHelper::isEmailAddress($recipientOthers)) {
            return 0;
        }
        ...
        try {
            ...
            if (!$body) {
                $body = Text::_('COM_PHOCACART_ORDER_NR') . ': ' . $orderNumber . ' - ' . Text::_('COM_PHOCACART_ORDER_STATUS_CHANGED_TO') . ': ' . $status['title'] . '<br>';
            }
            ...
            // REPLACE
            $r                 = PhocacartText::prepareReplaceText($orderView, $order->id, $order, $addresses, $status);
            ...
            $body       = PhocacartText::completeTextFormFields($body, $addresses['b'], $addresses['s']);
            $bodyOthers = PhocacartText::completeTextFormFields($bodyOthers, $addresses['b'], $addresses['s']);

            switch ($emailSend) {
                case 1:
                    $orderRender = new PhocacartOrderRender();

                    if ($emailSendFormat == 0 || $emailSendFormat == 2) {
                        $body .= "<br><br>";
                        $body .= $orderRender->render($order->id, 1, 'mail', $orderToken);

                        $bodyOthers .= "<br><br>";
                        $bodyOthers .= $orderRender->render($order->id, 1, 'mail', $orderToken);
                    }

                    if (Pdf::load() && in_array($emailSendFormat, [1, 2])) {
                        $orderNumber       = PhocacartOrder::getOrderNumber($order->id, $order->date, $order->order_number);
                        $attachmentName    = strip_tags(Text::_('COM_PHOCACART_ORDER') . '_' . $orderNumber) . '.pdf';
                        $attachmentContent = Pdf::renderPdf([
                            'title'    => Text::_('COM_PHOCACART_ORDER_NR') . ': ' . $orderNumber,
                            'filename' => $attachmentName,
                            'output'   => $orderRender->render($order->id, 1, 'pdf', $orderToken),
                        ]);
                    }
                ...
                case 2:
                    ...
                    $invoiceNumber = PhocacartOrder::getInvoiceNumber($order->id, $order->date, $order->invoice_number);


                    // If invoice is not created yet, it cannot be sent
                    if ($invoiceNumber == '') {
                        PhocacartLog::add(3, 'Status changed - sending email: The invoice should have been attached to the email, but it doesn not exist yet. Check order status settings and billing settings.', $order->id, 'Order ID: ' . $order->id . ', Status ID: ' . $status['id']);
                    } else {
                        if ($emailSendFormat == 0 || $emailSendFormat == 2) {
                            $body .= "<br><br>";
                            $body .= $orderRender->render($order->id, 2, 'mail', $orderToken);

                            $bodyOthers .= "<br><br>";
                            $bodyOthers .= $orderRender->render($order->id, 2, 'mail', $orderToken);
                        }

                        if (Pdf::load() && in_array($emailSendFormat, [1, 2])) {
                            $attachmentName    = strip_tags(Text::_('COM_PHOCACART_INVOICE') . '_' . $invoiceNumber) . '.pdf';
                            $attachmentContent = Pdf::renderPdf([
                                'title'    => Text::_('COM_PHOCACART_INVOICE_NR') . ': ' . $invoiceNumber,
                                'filename' => $attachmentName,
                                'output'   => $orderRender->render($order->id, 2, 'pdf', $orderToken),
                            ]);
                        }
                    }
                ...
                case 3:
                    ...
                    if ($emailSendFormat == 0 || $emailSendFormat == 2) {
                        $body .= "<br><br>";
                        $body .= $orderRender->render($order->id, 3, 'mail', $orderToken);

                        $bodyOthers .= "<br><br>";
                        $bodyOthers .= $orderRender->render($order->id, 3, 'mail', $orderToken);
                    }
                    ...
                    if (Pdf::load() && in_array($emailSendFormat, [1, 2])) {
                        ...
                        $attachmentContent = Pdf::renderPdf([
                            'title'    => Text::_('COM_PHOCACART_ORDER_NR') . ': ' . $orderNumber,
                            'filename' => $attachmentName,
                            'output'   => $orderRender->render($order->id, 3, 'pdf', $orderToken),
                        ]);
                    }
        ...
        if ($recipientOthers != '' && MailHelper::isEmailAddress($recipientOthers)) {
            PhocacartEmail::sendEmail('', '', $recipientOthers, $subjectOthers, $bodyOthers, true, null, $bcc, null, $attachmentContent, $attachmentName);
        }
    ...
    private static function sendGiftEmail(object $order, PhocacartOrderView $orderView, array $status, $addresses, $orderToken): void
    {
        ...
        if (in_array($status['email_gift'], [2, 3])) {
            foreach ($gifts as $gift) {
                ...
                if (MailHelper::isEmailAddress($gift['gift_recipient_email'] ?? '')) {
        ...
        if ($buyerEmail) {
            ...
            if ($status['email_subject_gift_sender'] != '') {
                ...
                $buyerSubject = $config->get('sitename') . ' - ' . $status['title'] . ' ' . Text::_('COM_PHOCACART_ORDER_NR') . ': ' . $r['ordernumber'] . ' - ' . $giftVoucherText;
            }
    ...
    {
		if (MailHelper::isEmailAddress($addresses['b']['email_contact'] ?? '')) {
			return $addresses['b']['email_contact'];
		} else if (MailHelper::isEmailAddress($addresses['b']['email'] ?? '')) {
			return $addresses['b']['email'];
		} else if (MailHelper::isEmailAddress($addresses['s']['email_contact'] ?? '')) {
			return $addresses['s']['email_contact'];
		} else if (MailHelper::isEmailAddress($addresses['s']['email'] ?? '')) {
