...
class JFormFieldPhocaText extends JFormFieldText
{
	...
	protected function getInput() {
		...
		if (!$this->hidden && ($this->form->getValue('version') == 1)) {
			...
			//$class		= $this->element['class'] ? ' class="'.(string) $this->element['class'].' form-control"' : 'class="form-control"';//Bootstrap3
			...
			if ($postIcon && $this->element['posthref']) {
				...
				$postIcon = '<span class="add-on input-group-addon"><a href="' . (string) $this->element['posthref'] . '" title="' . JText::_('COM_PHOCAGUESTBOOK_RELOAD_IMAGE') . '" class="" >' . $postIcon . '</a></span>';
			}
			...
			$postIcon	= $this->element['posticon'] 	? '<i class="' . $this->element['posticon'] . '"></i>' : '';
			if ($postIcon && $this->element['posthref']) {
				$postIcon = '<a href="' . (string) $this->element['posthref'] . '" title="' . JText::_('COM_PHOCAGUESTBOOK_RELOAD_IMAGE') . '" class="btn " >' . $postIcon . '</a>';
			}
