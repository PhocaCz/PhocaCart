...
$modalClass       = isset($d['modal-class']) ? $d['modal-class'] : 'modal-lg';// Call the style class and find the class per key
$modalCustomClass = isset($d['modal-custom-class']) ? $d['modal-custom-class'] : '';// Writes directly the class

?>
<div id="<?php echo $d['id'] ?>" class="<?php echo $s['c']['modal.zoom'] ?>" <?php echo $s['a']['modal'] ?> role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display:none">
    <div class="<?php echo $s['c']['modal-dialog'] ?> <?php echo $s['c'][$modalClass] ?> <?php echo $modalCustomClass ?>">
        <div class="<?php echo $s['c']['modal-content'] ?>">
            ...
            <div class="<?php echo $s['c']['modal-footer'] ?>"><?php
                if (isset($d['closebutton']) && $d['closebutton'] == 1) {
                    echo '<button type="button" class="' . $s['c']['btn.btn-secondary'] . ' ' . $s['c']['modal-common-close'] . '" ' . $d['s']['a']['data-bs-dismiss-modal'] . ' >' . Text::_('COM_PHOCACART_CLOSE') . '</button>';
                }
                ?></div>
        </div>
    </div>
