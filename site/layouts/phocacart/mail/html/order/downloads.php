...
if ($downloads) {
?>
    <h4><?= Text::_('COM_PHOCACART_DOWNLOAD_LINKS') ?></h4>
    <?php foreach ($downloads as $productDownloads) { ?>
      <h5 style="margin: 0"><?= $productDownloads['title'] ?></h5>
      <ul style="margin: 0">
        <?php foreach ($productDownloads['downloads'] as $download) { ?>
          ...
          ?>
          <li style="margin: 0"><a href="<?= $downloadLink ?>" target="_blank"><?= $download['title'] ?></a></li>
        <?php } ?>
      </ul>
    <?php } ?>
<?php
