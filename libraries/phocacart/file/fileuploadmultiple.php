...
class PhocaCartFileUploadMultiple
{
	...
	static public function renderMultipleUploadLibraries() {
		...
		// No more used  - - - - -
		//$document->addScript(JURI::root(true).'/components/com_phocacart/assets/jquery/jquery-1.6.4.min.js');//USE SYSTEM
		//$nC = 'var pgJQ =  jQuery.noConflict();';//SET BELOW
		//$document->addScriptDeclaration($nC);//SET BELOW
		// - - - - - - - - - - - -
		//$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.js');
		//$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/jquery.ui.plupload/jquery-ui.min.js');
		//$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/jquery.ui.plupload/jquery.ui.plupload.js');
		
		if ($uploadMethod == 2) {
			//$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/gears_init.js');
		}
		if ($uploadMethod == 5) {
			//$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.browserplus.js');
		}
		
		$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.js');
		if ($uploadMethod == 2) {
			$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.gears.js');
		}
		if ($uploadMethod == 3) {
			$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.silverlight.js');
		}
		if ($uploadMethod == 1) {
			$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.flash.js');
		}
		if ($uploadMethod == 5) {
			$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.browserplus.js');
		}
		if ($uploadMethod == 6) {
			$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.html4.js');
		}
		if ($uploadMethod == 4) {
			$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.html5.js');
		}
		$document->addScript(JURI::root(true).'/components/com_phocacart/assets/plupload/jquery.plupload.queue/jquery.plupload.queue.js');
		JHTML::stylesheet( 'components/com_phocacart/assets/plupload/jquery.plupload.queue/css/jquery.plupload.queue.css' );
	}
	...
	public function renderMultipleUploadJS($frontEnd = 0, $chunkMethod = 0) {
		...
		$js.='		multipart: true,'."\n";
		$js.='		filters : ['."\n";
		//$js.='			{title : "'.JText::_('com_phocacart_IMAGE_FILES').'", extensions : "jpg,gif,png"}'."\n";
		...
		if ($this->method == 1) {
			$js.='		flash_swf_url : \''.JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.flash.swf\''."\n";
		} else if ($this->method == 3) {
			$js.='		silverlight_xap_url : \''.JURI::root(true).'/components/com_phocacart/assets/plupload/plupload.silverlight.xap\''."\n";
		}
		$js.='	});'."\n";
