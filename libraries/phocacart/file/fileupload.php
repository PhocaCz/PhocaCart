...
class PhocaCartFileUpload
{
	public static function realMultipleUpload( $frontEnd = 0) {
		...
		if (isset($file['name'])) {
			...
			if ($chunkEnabled == 1) {
				...
				$tempFolder				= 'pcpluploadtmpfolder'.DS;
	...
	//	$uploadMethod 	= $paramsC->get( 'multiple_upload_method', 1 );
		...
		JRequest::checkToken( 'request' ) or jexit( 'ERROR: '. JTEXT::_('COM_PHOCACART_INVALID_TOKEN'));
		JResponse::allowCache(false);
	...
	public static function canUpload( $file, &$err, $manager = '', $frontEnd = 0, $chunkEnabled = 0, $realSize = 0) {
		...
		if ($frontEnd == 1) {
			$aft = $paramsC->get( 'allowed_file_types_upload', PhocaCartSettings::getDefaultAllowedMimeTypesUpload() );
			...
			$aft = $paramsC->get( 'allowed_file_types_download', PhocaCartSettings::getDefaultAllowedMimeTypesDownload() );
		...
		$html_tags = PhocaCartSettings::getHTMLTagsUpload();
